import numpy as np
import tensorflow.compat.v1 as tf1  # pylint: ignore-module
import copy
import os
import functools
import collections
...
def conv2d(x, num_filters, name, filter_size=(3, 3), stride=(1, 1), pad="SAME", dtype=tf1.float32, collections=None,
           summary_tag=None):
    with tf1.variable_scope(name):
        ...
        w = tf1.get_variable("W", filter_shape, dtype, tf1.random_uniform_initializer(-w_bound, w_bound),
                            collections=collections)
        b = tf1.get_variable("b", [1, 1, 1, num_filters], initializer=tf1.zeros_initializer(),
                            collections=collections)

        if summary_tag is not None:
            tf1.summary.image(summary_tag,
                             tf1.transpose(tf1.reshape(w, [filter_size[0], filter_size[1], -1, 1]),
                                          [2, 0, 1, 3]),
                             max_images=10)
...
def function(inputs, outputs, updates=None, givens=None):
    ...
    Example:
        x = tf1.placeholder(tf1.int32, (), name="x")
        y = tf1.placeholder(tf1.int32, (), name="y")
        z = 3 * x + 2 * y
    ...
    Parameters
    ----------
    inputs: [tf1.placeholder, tf1.constant, or object with make_feed_dict method]
        list of input arguments
    outputs: [tf1.Variable] or tf1.Variable
        list of outputs or a single output to be returned from function. Returned
        value will also have the same shape.
    updates: [tf1.Operation] or tf1.Operation
...
class _Function(object):
    def __init__(self, inputs, outputs, updates, givens):
        for inpt in inputs:
            if not hasattr(inpt, 'make_feed_dict') and not (type(inpt) is tf1.Tensor and len(inpt.op.inputs) == 0):
...
def flatgrad(loss, var_list, clip_norm=None):
    ...
    if clip_norm is not None:
        grads = [tf1.clip_by_norm(grad, clip_norm=clip_norm) for grad in grads]
    return tf1.concat(axis=0, values=[
        tf1.reshape(grad if grad is not None else tf1.zeros_like(v), [numel(v)])
...
class SetFromFlat(object):
    ...
    def __call__(self, theta):
        tf1.get_default_session().run(self.op, feed_dict={self.theta: theta})

class GetFlat(object):
    def __init__(self, var_list):
        self.op = tf1.concat(axis=0, values=[tf1.reshape(v, [numel(v)]) for v in var_list])

    def __call__(self):
        return tf1.get_default_session().run(self.op)
...
def get_placeholder(name, dtype, shape):
    if name in _PLACEHOLDER_CACHE:
        ...
        if out.graph == tf1.get_default_graph():
...
def load_state(fname, sess=None):
    ...
    saver.restore(tf1.get_default_session(), fname)
...
def save_state(fname, sess=None):
    ...
    saver = tf1.train.Saver()
    saver.save(tf1.get_default_session(), fname)
...
def save_variables(save_path, variables=None, sess=None):
    import joblib
    sess = sess or get_session()
    variables = variables or tf1.get_collection(tf1.GraphKeys.GLOBAL_VARIABLES)
...
# ================================================================
# Shape adjustment for feeding into tf1 placeholders
# ================================================================
...
def launch_tensorboard_in_background(log_dir):
    ...
    in your main script:
        ...
        def start_tensorboard(session):
            ...
            summary_writer = tf1.summary.FileWriter(tb_path, graph=session.graph)
